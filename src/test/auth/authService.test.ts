...
describe('AuthService', () => {
    ...
    describe('startAuthentication', () => {
        ...
        it('should send AUTH_REQ when server-initiated', () => {
            ...
            expect(mockSendMessage).toHaveBeenCalledWith(
                expect.stringContaining(AUTH.MESSAGE_TYPES.AUTH_REQ)
            );
        });
    });
    ...
    describe('handleMessage', () => {
        ...
        it('should handle AUTH_RESP messages', () => {
            ...
            (authService as any).states.get(connectionId).state = 'WAITING_FOR_AUTH_RESP';
        ...
        it('should handle AUTH_SUCCESS messages', () => {
            ...
            (authService as any).states.get(connectionId).state = 'WAITING_FOR_AUTH_SUCCESS_FAIL';
    ...
    describe('provideUserInput', () => {
        ...
        it('should send AUTH_RESP when providing input for challenge', () => {
            ...
            (authService as any).states.get(connectionId).state = 'WAITING_FOR_USER_INPUT';

            authService.provideUserInput(connectionId, ['elephant', 'tiger', 'giraffe']);

            expect(mockSendMessage).toHaveBeenCalledWith(
                expect.stringContaining(AUTH.MESSAGE_TYPES.AUTH_RESP)
            );
        });

        it('should validate response and send AUTH_SUCCESS when valid', () => {
            // Change state to waiting for user input validation
            (authService as any).states.get(connectionId).state = 'WAITING_FOR_USER_INPUT_VALIDATION';
            (authService as any).states.get(connectionId).pendingResponse = ['elephant', 'tiger', 'giraffe'];
            ...
            expect(mockSendMessage).toHaveBeenCalledWith(
                expect.stringContaining(AUTH.MESSAGE_TYPES.AUTH_SUCCESS)
            );
            expect(spy).toHaveBeenCalledWith('authSucceeded', connectionId);
        });
    });
