...
describe('AuthProcess', () => {
    ...
    let mockTimeoutHandle: NodeJS.Timeout;
    ...
    beforeEach(() => {
        // Create mocks
        mockTimeoutHandle = {} as NodeJS.Timeout;
    ...
    describe('start', () => {
        it('should set a timeout', () => {
            ...
            // Assert
            expect(mockTimer.set).toHaveBeenCalledWith(expect.any(Function), AUTH.TIMEOUT);
        });
        ...
        it('should send an auth request if initiator', () => {
            ...
            // Assert
            expect(mockSendMessage).toHaveBeenCalledWith(expect.stringContaining(AUTH.MESSAGE_TYPES.AUTH_REQ));
    ...
    describe('handleMessage', () => {
        ...
        it('should handle AUTH_REQ message when in AWAITING_REQ state', () => {
            ...
            // Act
            authProcess.handleMessage({ type: AUTH.MESSAGE_TYPES.AUTH_REQ });
            
            // Assert
            expect(mockSendMessage).toHaveBeenCalledWith(expect.stringContaining(AUTH.MESSAGE_TYPES.AUTH_RESP));
        });
        ...
        it('should fail if AUTH_REQ received in wrong state', () => {
            ...
            // Assert
            expect(mockSendMessage).toHaveBeenCalledWith(expect.stringContaining(AUTH.MESSAGE_TYPES.AUTH_FAIL));
        ...
        it('should handle AUTH_RESP message when in AWAITING_RESP state', () => {
            // Act
            authProcess.handleMessage({ type: AUTH.MESSAGE_TYPES.AUTH_RESP });
            
            // Assert
            expect(mockSendMessage).toHaveBeenCalledWith(expect.stringContaining(AUTH.MESSAGE_TYPES.AUTH_SUCCESS));
        ...
        it('should handle AUTH_SUCCESS message when in AWAITING_SUCCESS state', () => {
            ...
            authProcess.start(); // Sets state to AWAITING_REQ
            authProcess.handleMessage({ type: AUTH.MESSAGE_TYPES.AUTH_REQ }); // Sets state to AWAITING_SUCCESS
            mockSendMessage.mockClear();
            
            // Act
            authProcess.handleMessage({ type: AUTH.MESSAGE_TYPES.AUTH_SUCCESS });
        ...
        it('should ignore messages if already done', () => {
            // Arrange
            authProcess.handleMessage({ type: AUTH.MESSAGE_TYPES.AUTH_RESP }); // Completes auth
            mockSendMessage.mockClear();
