...
import { ActiveConnectionCoordinator } from './ActiveConnectionCoordinator';
...
describe('ActiveConnectionCoordinator', () => {
    // Mock dependencies
    let mockConnectionHandlerFactory: jest.Mocked<IConnectionHandlerFactory>;
    let mockLogger: jest.Mocked<ILogger>;
    let mockAuthService: jest.Mocked<IAuthService>;
    let mockConnectionHandler: jest.Mocked<IConnectionHandler>;
    ...
    // System under test
    let coordinator: ActiveConnectionCoordinator;
    
    beforeEach(() => {
        // Create mocks
        mockConnectionHandler = new EventEmitter() as jest.Mocked<IConnectionHandler>;
        ...
        mockAuthService = new EventEmitter() as jest.Mocked<IAuthService>;
        mockAuthService.startAuthentication = jest.fn();
        
        mockSocket = new EventEmitter() as jest.Mocked<net.Socket>;
        mockSocket.remoteAddress = '127.0.0.1';
        mockSocket.remotePort = 12345;
        mockSocket.destroy = jest.fn();
        
        // Create system under test
        coordinator = new ActiveConnectionCoordinator(
            mockConnectionHandlerFactory,
            mockAuthService,
            mockLogger
        );
    ...
    describe('registerOutgoingConnection', () => {
        ...
        it('should create a connection handler and start authentication', () => {
            ...
            // Assert
            expect(mockConnectionHandlerFactory.create).toHaveBeenCalledWith(
                mockSocket,
                mockLogger,
                mockAuthService,
                true // isInitiator
